...
%以特定的key生成特定的随机矩阵来存放秘密图片的信息
npoints = row1*col1;
rand('state', key);
position = randperm(npoints);
...
if(row2*col2>(row1*col1-20))  %R层的前20LSB位存储秘密信息的宽和高信息
    ...
    %R层的随机矩阵前20个LSB位存储秘密信息的宽和高信息，只有R层存储信息，后面G层和B层就可以直接放了
    for i = 1:20
        R_(position(i)) = bitset(R_(position(i)),1,size_info(i));
    end
    %store the image info in the rest of cover;
    for j=1:row2*col2
        R_(position(j+20)) = bitor(R_(position(j+20)),shiftsecR(j));%将秘密图片的高三位放到载体图片R层的低3位
    end
    ...
    for k=1:row2*col2
        G_(position(k)) = bitor(G_(position(k)),shiftsecG(k));%将秘密图片的高三位放到载体图片的低3位
    end
    ...
    for l=1:row2*col2
        B_(position(l)) = bitor(B_(position(l)),shiftsecB(l));%将秘密图片的高三位放到载体图片的低3位
    end
